#!/usr/bin/env python3
"""Generate a horizontal bar chart of viewed hours by language.

Input JSON (via stdin):
{
  "data": [
    { "language": "en", "totalCCV": 159700, "avgCCV": 10647, "peakCCV": 21000 },
    { "language": "ko", "totalCCV": 82000, "avgCCV": 5467, "peakCCV": 12000 },
    ...
  ],
  "options": {
  ...
  "outputPath": "/tmp/charts/{id}/language_bars.png",
  "configPath": "..."
}
"""
...
def main():
    ...
    if not data:
        ...
        set_title(ax, options.get('title', 'Language Distribution'), config)
        save_chart(fig, output_path, config)
        return

    # Filter and sort
    filtered = [d for d in data if d.get('language')]
    filtered.sort(key=lambda d: d.get(metric, 0), reverse=True)

    labels = [(d['language'] or 'Unknown').upper() for d in filtered]
    values = [d.get(metric, 0) for d in filtered]
    grand_total = sum(values) or 1
    colors = get_color_cycle('language_colors', config, len(filtered))
